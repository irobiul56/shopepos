...
const submitCategory = () => {
    ...
    const routeName = isUpdate ? 'categories.update' : 'categories.store';
    const routeParams = isUpdate ? categoryForm.id : null;
    const method = isUpdate ? 'put' : 'post';

    categoryForm[method](route(routeName, routeParams), {
        onSuccess: (page) => {
            isModalVisible.value = false;
            categories.value = page.props.categories?.data || [];
            pagination.value = page.props.categories || {};
            categoryForm.reset();
            ElMessage.success({
...
const deleteCategory = () => {
    ...
    categoryForm.delete(route('categories.destroy', categoryToDelete.value), {
        ...
        onError: (errors) => {
            isDeleteModalVisible.value = false;
            ElMessage.error({
                message: errors.message || "Failed to delete the category.",
                customClass: 'error-message',
                duration: 3000
            });
        },
...
const toggleStatus = (category) => {
    categoryForm.put(route('categories.toggle-status', category.id), {
        onSuccess: (page) => {
            categories.value = page.props.categories?.data || [];
            ElMessage.success({
        ...
        <div class="min-h-screen w-full bg-gradient-to-br from-gray-50 to-blue-50 px-4 sm:px-6 lg:px-8 py-8">
            <div class="max-w-7xl mx-auto">
                ...
                <div class="mb-8">
                    ...
                    <p class="mt-2 text-gray-600">Manage your product categories efficiently</p>
                </div>
                ...
                <div class="inline-block w-full max-w-md p-6 my-8 overflow-hidden text-left align-middle transition-all transform bg-white shadow-xl rounded-2xl">
                    ...
                    <form @submit.prevent="submitCategory" class="space-y-6">
                        ...
                        <div>
                            ...
                            >
                                ...
                                <option 
                                    ...
                                    :key="parent.id" 
                                    :value="parent.id"
                                    :disabled="parent.id === categoryForm.id"
                                >
                                    {{ parent.name }}
                                </option>
                            </select>
                        ...
                        <div class="flex justify-end space-x-3 pt-4 border-t border-gray-200">
                            ...
                            >
                                <i v-if="categoryForm.processing" class="fas fa-spinner fa-spin mr-2"></i>
